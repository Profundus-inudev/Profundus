...
public class DatabaseUtil {
    ...
    public static void connect() {
        ...
        try {
            ...
            Profundus.getInstance().getLogger().severe(ExceptionUtils.getStackTrace(e));
        }
    }
 ...
 */
	...
	CHUNK,
	/**
	 * SQLテーブル名:TRANSACT.=TRANSACTIONは予約語のため避けた。
	 */
	TRANSACT;
    }
    ...
    public static boolean createTable(Table tableName, boolean dropIfExists) {
    	...
    	case MONEY:
    		...
    		break;
    	case CHUNK:
    		sql.append(DBUChunk.createStr);
    		break;
    	case TRANSACT:
    		sql.append(DBUTransaction.createStr);
    		break;
    	}
    	sql.append(");");
	    ...
	    try {
	    	...
	    	if(dropIfExists) {
		    	...
		    	con.commit();
		        Profundus.getInstance().getLogger().info("DropTable  : " + tableName.name());
		    	st.close();
	    	}
	        ...
	        con.commit();
	        st.close();
			Profundus.getInstance().getLogger().info("Table created/exists : " + tableName.name() );
	        return true;
	    } catch (SQLException e) {
	    	Profundus.getInstance().getLogger().warning(ExceptionUtils.getStackTrace(e));
	        try {
    ...
    static java.sql.ResultSet selectUUID(Table tableName, String idName, UUID uuid){
	    try {
	    	...
	    	if(tableName == Table.PFID) {
	    		...
	    		//PFIDテーブルの検索には，高速化のために，あらかじめ用意したSQLを使用。
	    	}else {
	           	Connection con = getConnection();
